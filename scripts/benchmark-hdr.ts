...
import { processPixels } from '../src/lib/pq'
...
const baselineConfig: BenchConfig = {
  name: 'baseline-compression-stream-l6',
  backend: 'compression-stream',
  level: 6,
}

const candidateConfigs: BenchConfig[] = [4, 5, 6, 7, 8, 9].map((level) => ({
  name: `fflate-l${level}`,
  backend: 'fflate',
  level,
}))
...
function toImageDataLike(data: Uint8ClampedArray, width: number, height: number): ImageData {
...
async function runCase(entry: CorpusEntry, config: BenchConfig): Promise<CaseResult> {
  ...
  const pq = processPixels(toImageDataLike(pixels, entry.width, entry.height), 5.0, 1.0)
...
async function runConfig(config: BenchConfig): Promise<ConfigSummary> {
  ...
  for (const entry of corpus) {
    const result = await runCase(entry, config)
...
async function main(): Promise<void> {
  ...
  await writeFile('benchmarks/hdr-benchmark-report.json', JSON.stringify(report, null, 2))
  console.log('Wrote benchmarks/hdr-benchmark-report.json')
}
