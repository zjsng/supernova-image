...
- **Display → headlines and metric values.** Any number the user is meant to *read* as a value (nits, multiplier, filename) is display.
...
@media (prefers-reduced-motion: reduce) {
  *, *::before, *::after {
  ...
  .hdr-bloom, .scan-bar, .not-found__star::before { animation: none !important; }
}
```
